...
for (const [chave, valor] of Object.entries(node.attrs || {})) {
const txt = String(valor || '').trim()
if (/^https?:\/\//i.test(txt)) return txt
if (/(url|path)/i.test(chave) && txt.startsWith('/')) return `https://pps.whatsapp.net${txt}`
}

if (typeof node.content === 'string' || Buffer.isBuffer(node.content)) {
const txt = node.content.toString().trim()
if (/^https?:\/\//i.test(txt)) return txt
if (txt.startsWith('/')) return `https://pps.whatsapp.net${txt}`
}

if (Array.isArray(node.content)) {
for (const filho of node.content) {
const url = pegarUrl(filho)
if (url) return url
}
}
...
const websites = getBinaryNodeChildren(profiles, 'website')
...
const cover = achar(profiles, 'cover_photo')
const coverPhotoUrl = pegarUrl(cover)

return {
wid: profiles.attrs?.jid,
address: address?.content?.toString(),
description: description?.content?.toString() || '',
website: websites.map(site => site?.content?.toString()).filter(Boolean),
email: email?.content?.toString(),
category: category?.content?.toString(),
coverPhotoUrl,
cover_photo: cover ? {...cover.attrs, url: coverPhotoUrl} : undefined,
business_hours: {
