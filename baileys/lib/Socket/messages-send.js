...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            {
                const normalizedInteractiveContent = normalizeMessageContent(message);
                const normalizedInteractiveType = normalizedInteractiveContent ? Object.keys(normalizedInteractiveContent)[0] : '';
                if ((isJidGroup(jid) || server === 's.whatsapp.net' || isLid) && (normalizedInteractiveType === 'interactiveMessage' || normalizedInteractiveType === 'buttonsMessage' || normalizedInteractiveType === 'listMessage')) {
                    const bizNode = { tag: 'biz', attrs: {} };
                    if (normalizedInteractiveType === 'interactiveMessage' || normalizedInteractiveType === 'buttonsMessage') {
                        bizNode.content = [{
                                tag: 'interactive',
                                attrs: {
                                    type: 'native_flow',
                    ...
                    else if (normalizedInteractiveType === 'listMessage') {
                        bizNode.content = [{
                                tag: 'list',
                                attrs: {
                                    type: 'product_list',
